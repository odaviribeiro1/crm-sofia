  ...
  try {
    ...
    if (instance.provider_type !== 'official' && secrets?.api_url && secrets?.api_key) {
      try {
        ...
        const deleteRes = await fetch(`${baseUrl}/instance`, {
          method: 'DELETE',
    ...
    return new Response(JSON.stringify({ success: true, uazapi_deleted: uazapiDeleted, uazapi_error: uazapiError }), {

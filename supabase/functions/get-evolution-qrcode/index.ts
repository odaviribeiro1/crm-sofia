...
serve(async (req) => {
  ...
  try {
    ...
    const stateRes = await fetch(`${baseUrl}/instance/status`, { method: 'GET', headers: instanceHeaders });
    ...
    // Buscar QR Code
    const qrRes = await fetch(`${baseUrl}/instance/qr`, { method: 'GET', headers: instanceHeaders });
